    ...
    public Database() {
        try {
            if(Config.getConfig().getBoolean("database.mariadb")) {
                ...
                conn.prepareStatement("CREATE TABLE IF NOT EXISTS players(uuid TEXT, name TEXT, nickname TEXT, timesjoined long, playtime long, randomitem BOOLEAN, chatcolor TEXT, gradient BOOLEAN, gradientfrom TEXT, gradientto TEXT, night BOOLEAN, balance BIGINT, deathmessages BOOLEAN)").execute();
                    ...
                    try(ResultSet set = conn.prepareStatement("SELECT * FROM players").executeQuery()) {
                        if(!set.next()) {
                            ...
                            PreparedStatement stat = conn.prepareStatement("INSERT INTO" +
                                    " players(uuid, name, nickname, timesjoined, playtime, randomitem, chatcolor, gradient, gradientfrom, gradientto, night, balance, deathmessages)" +
                                    " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                playerConn.prepareStatement("CREATE TABLE IF NOT EXISTS players(uuid varchar, name varchar, nickname varchar, timesjoined long, playtime long, randomitem BOOLEAN, chatcolor VARCHAR, gradient BOOLEAN, gradientfrom VARCHAR, gradientto VARCHAR, night BOOLEAN, balance BIGINT, deathmessages VARCHAR)").execute();
