...
public class RandomCMD implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if(to) {
            player.sendMessage(Utils.format("&aTurned on random items. You will now recieve a random item every 60 seconds."));
            JoinEvent.randomItemList.add(player);
        } else {
            player.sendMessage(Utils.format("&aTurned off random items. You will no longer recieve a random item every 60 seconds."));
